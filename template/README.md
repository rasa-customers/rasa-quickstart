...
| `make actions` | Run the custom action server |
...
```
config.yml          # CALM pipeline + LLM model group (edit to change provider/model)
domain/             # responses, slots, actions the agent knows about
data/flows/         # the conversation flows (business logic)
actions/            # custom Python actions
endpoints.yml       # action server, tracker store, model server
...
- **Switch LLM provider:** edit the `model_groups` block in `config.yml` and set the matching key in `.env`.
